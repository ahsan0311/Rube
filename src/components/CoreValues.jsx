...
import { FaLightbulb, FaHandshake, FaChartLine, FaUserGraduate } from "react-icons/fa"; // Icons

const coreValues = [
  {
    icon: <FaUserGraduate className="text-green-500 text-5xl mx-auto mb-4" />,
  ...
  {
    icon: <FaLightbulb className="text-blue-500 text-5xl mx-auto mb-4" />,
    title: "Innovation",
    items: [
  ...
  {
    icon: <FaChartLine className="text-purple-600 text-5xl mx-auto mb-4" />,
    title: "Empowerment",
    items: [
    ...
    <section className="relative bg-white pt-16 pb-24 px-6 text-center overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {coreValues.map(
            ({ icon, title, items, borderColor, textColor, borderWidth }, i) => (
              <div
                key={i}
                className={`${borderColor} ${borderWidth} rounded-lg p-6 shadow-md hover:shadow-xl transition-shadow duration-300 bg-white`}
              >
                {/* Icon */}
                {icon}
